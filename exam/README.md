...
> **Important**: Как известно, в С++ ключевое слово virtual используется для объявления виртуальных функций, которые будут переопределены в производных классах (**СМОТРИТЕ БИЛЕТ 7 и 61!!!**)
...
То есть эта функция которая просто существует в базовом классе, но её реализация может отличаться в разных дочерних классах (пример полиморфизма - возможность одного и того же кода работать с РАЗНЫМИ типами данных)
