...
### 9. Встраиваемая функция.
Основная идея в том, чтобы ускорить программу ценой занимаемого места.

> **Note**: Смотрите также БИЛЕТ 17!

**inline** функции при вызове заменяются компилятором фактическим кодом из функции (вместо самого вызова функции).
