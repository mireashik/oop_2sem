...
## 11. Дружественная функция.
**Дружественные функции** - это функции, которые **НЕ** являются членами класса, однако **ИМЕЮТ** доступ к его **private и protected** членам. Для определения дружественных функций используется ключевое слово `friend`.
...
## 20. Класс vector.
> **Note**: АНАЛОГИЧНО ТЕМЕ В БИЛЕТЕ 26. Контейнер – динамический массив.
...
Конструктор копирования должен принимать в качестве параметра объект **одного** класса. Причем параметр лучше принимать по **ссылке &**, потому что при передаче по значению компилятор будет создавать копию объекта. А для создания копия объекта будет вызываться конструктор копирования, что приведет бесконечной рекурсии. Итак, определим свой конструктор копирования:
...
1 итератор = 2 итератору, если они указывают на 1 и тот же элемент
...
Виды конструкторов:
- **Конструктор по умолчанию**. Если программист не включил в класс ни 1 конструктор, то компилятор создаст его автоматически, и он будет вызывается автоматически при определении объекта.
...
Деструкторы не наследуется! 

## 31. Класс map и multimap
> **Note**: АНАЛОГИЧНО ТЕМЕ В БИЛЕТЕ 28. Контейнер – ассоциативный список
